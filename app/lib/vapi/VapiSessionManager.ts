...
import { isTransientVapiError, logVapi } from "@/app/lib/vapiConfig";

export const VAPI_CONNECT_TIMEOUT_MS = 45_000;

const GLOBAL_SINGLETON_KEY = "__infiniaVapiSessionManager__";
...
export class VapiSessionManager {
  ...
  static getInstance(): VapiSessionManager {
    if (typeof globalThis !== "undefined") {
      ...
      const existing = globalRecord[GLOBAL_SINGLETON_KEY];
  ...
  async ensureClient(publicKey: string): Promise<Vapi> {
    if (this.client && this.publicKey === publicKey) {
      return this.client;
    }
    if (this.client) {
      logVapi("info", "client-recreate", { reason: "public-key-changed" });
  ...
  private safeOn(
    ...
    if (!this.client || typeof listener !== "function") {
      return;
    }
    try {
      (this.client.on as (event: string, listener: (...args: any[]) => void) => void)(
        event,
        listener
      );
    } catch {
  ...
  private safeOff(
    ...
    try {
      (this.client.removeListener as (event: string, listener: (...args: any[]) => void) => void)(
        event,
        listener
      );
    } catch {
  ...
  attachHandlers(handlers: VapiSessionHandlers): void {
    ...
    this.safeOn("call-start", handlers.onCallStart);
    this.safeOn("message", handlers.onMessage);
    this.safeOn("call-end", handlers.onCallEnd);
    this.safeOn("error", handlers.onError);
    this.safeOn("call-start-failed", handlers.onCallStartFailed);
  }
  ...
  detachHandlers(): void {
    ...
    this.safeOff("call-start", h.onCallStart);
    this.safeOff("message", h.onMessage);
    this.safeOff("call-end", h.onCallEnd);
    this.safeOff("error", h.onError);
    this.safeOff("call-start-failed", h.onCallStartFailed);
  ...
  async startCall(assistantId: string): Promise<void> {
    ...
    this.startPromise = this.runStartCall(assistantId).finally(() => {
      this.startPromise = null;
    });
    return this.startPromise;
  }

  private async runStartCall(assistantId: string): Promise<void> {
    if (!this.client) {
      throw new Error("Vapi client is not initialized.");
    }
    ...
    logVapi("info", "start-call", { assistantId, generation });
    try {
      ...
      logVapi("warn", "start-retry-transient", { generation, error: firstError });
      await this.stopCall("transient-retry-preflight");
      await sleep(TRANSIENT_RETRY_DELAY_MS);
      await this.waitForStop();
      if (this.callGeneration !== generation) {
        throw new Error("Call generation changed during retry.");
      }
      await this.client.start(assistantId);
    }
    logVapi("info", "start-call-invoked", { generation });
  }
    ...
    this.stopPromise = this.client
      ...
      .catch((err) => {
        if (!isHarmlessTeardownError(err)) {
          logVapi("warn", "stop-call-error", { reason, error: err });
        }
      })
      .finally(() => {
